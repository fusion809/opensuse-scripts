...
function messengerfordesktop-install {
  # printf "${MESSENGER_LATEST_VERSION}" >> $HOME/.local/share/messengerfordesktop-${MESSENGER_LATEST_VERSION} is in case the Atom package is broken like Atom 1.12.8
	zpi https://github.com/Aluxian/Messenger-for-Desktop/releases/download/v${MESSENGER_LATEST_VERSION}/messengerfordesktop-${MESSENGER_LATEST_VERSION}-linux-x86_64.rpm || printf "${MESSENGER_LATEST_VERSION}" >> $HOME/.local/share/messengerfordesktop-${MESSENGER_LATEST_VERSION}
}
