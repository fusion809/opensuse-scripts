...
function ps_gist {
  if [[ -n $2 ]]; then
    ps_mem ${$(pgrep $1 | tr '\n' ,)%,*},$2 > "$1-opensuse-psmem.sh" && gist "$1-opensuse-psmem.sh" && rm "$1-opensuse-psmem.sh"
  elif [[ -n $1 ]]; then
    ps_apl "$1" > "$1-opensuse-psmem.sh" && gist "$1-opensuse-psmem.sh" && rm "$1-opensuse-psmem.sh"
  fi
}
...
function ps_gistv {
  ...
  if [[ -n "$DESC" ]]; then
    ...
    echo "$Q" > "$DESC-opensuse-psmem.sh" && gist "$DESC-opensuse-psmem.sh" && rm "$DESC-opensuse-psmem.sh"
  fi
}

function ps_gistid {
  ps_mem $@ >> opensuse-psmem.sh
}
